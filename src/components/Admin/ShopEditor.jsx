...
function ShopEditor() {
    ...
    useEffect(() => {
        ...
        // 2. Handle the "Toggle Sold" for existing items
        const toggleSoldStatus = async (id, currentStatus) => {
            ...
            else fetchShopItems(); // Refresh the list
        };
    ...
    return (
        <div className={formStyles.adminSectionWrapper}>
            <form className={formStyles.editorForm} onSubmit={handleSubmit}>
                ...
                {/* SOLD TOGGLE SECTION */}
                <div className={formStyles.inputGroup}>
                    <label className={formStyles.checkboxLabel} style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}>
                        <input
                            type="checkbox"
                            checked={item.isSold}
                            onChange={e => setItem({ ...item, isSold: e.target.checked })}
                            style={{ width: '20px', height: '20px' }}
                        />
                        <span>Mark as SOLD immediately?</span>
                    </label>
                </div>
            ...
            <div className={formStyles.recentPosts}>
                ...
                <div className={formStyles.postGrid}>
                    {allShopItems.map((i) => (
                        ...
                        {/* Dynamic styling for price if sold */}
                        <p className={i.isSold ? formStyles.priceSold : formStyles.priceText}>
                            ${i.price} {i.isSold ? "(SOLD)" : ""}
                        </p>
                        
                        <div className={formStyles.adminActions}>
                            {/* THE MISSING BUTTON */}
                            <button 
                                type="button"
                                onClick={() => toggleSoldStatus(i.id, i.isSold)}
                                className={i.isSold ? formStyles.availableBtn : formStyles.soldBtn}
                            >
                                {i.isSold ? "Mark Available" : "Mark Sold"}
                            </button>
